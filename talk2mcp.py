    ...
    try:
        ...
        async with stdio_client(server_params) as (read, write):
            ...
            async with ClientSession(read, write) as session:
                ...
                system_prompt = """You are a math agent solving problems in iterations. You must think step-by-step and verify your logic before taking action. You have access to various mathematical tools.
...
# Your entire response should be a single line starting with either FUNCTION_CALL: or FINAL_ANSWER: or GUI_CALL: or AUTOMATION_DONE:"""
                ...
                while iteration < max_iterations:
                    ...
                    # print(f"Response text 111: {response_text}")
                    # print("11"*50)
                    if response_text.startswith("FUNCTION_CALL:"):
                        ...
                        try:
                            ...
                            if not tool:
                                ...
                                error_msg = f"Unknown tool: {func_name}. Please use only the available tools."
                                print(f"DEBUG: Error details: {error_msg}")
                                iteration_response.append(
                                    f"Error in iteration {iteration + 1}: {error_msg}. "
                                    f"Available tools are: {', '.join(t.name for t in tools)}"
                                )
                    ...
                    elif response_text.startswith("FINAL_ANSWER:") or "FINAL_ANSWER:" in response_text:
                        try:
                            final_answer = response_text.split(":")[1].strip()
                            print(f"FINAL_ANSWER: 1. {final_answer}")
                            # iteration_response.append(
                            #     f"The FINAL_ANSWER is {final_answer}"
                            # )
                            # Get the full result content
                            
                            iteration_response.append(
                                f"In the {iteration + 1} You got the FINAL_ANSWER {final_answer}"
                            )
                            last_response = f"The FINAL_ANSWER is {final_answer}"
